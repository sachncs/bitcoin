...
class TestSighashFlag:
    ...
    def test_require_valid_all_acp(self) -> None:
        assert (
            require_sighash_flag(SIGHASH_ALL_ANYONECANPAY)
            == SIGHASH_ALL_ANYONECANPAY
        )

    def test_require_valid_none_acp(self) -> None:
        assert (
            require_sighash_flag(SIGHASH_NONE_ANYONECANPAY)
            == SIGHASH_NONE_ANYONECANPAY
        )
...
class TestSighashSegwit:
    ...
    def test_all(self) -> None:
        h = sighash_segwit(
            TX_2IN_2OUT, 0, SCRIPT, self.VALUE, SIGHASH_ALL
        )
        assert len(h) == 32

    def test_none(self) -> None:
        h = sighash_segwit(
            TX_2IN_2OUT, 0, SCRIPT, self.VALUE, SIGHASH_NONE
        )
        assert len(h) == 32
    ...
    def test_single_input_index_zero(self) -> None:
        ...
        h = sighash_segwit(
            TX_2IN_2OUT, 0, SCRIPT, self.VALUE, SIGHASH_SINGLE
        )
        assert len(h) == 32
    ...
    def test_single_with_nonzero_index(self) -> None:
        ...
        h = sighash_segwit(
            TX_2IN_2OUT, 1, SCRIPT, self.VALUE, SIGHASH_SINGLE
        )
        assert len(h) == 32
    ...
    def test_single_out_of_range(self) -> None:
        ...
        with pytest.raises(ValueError, match="out of range"):
            sighash_segwit(
                tx, 1, SCRIPT, self.VALUE, SIGHASH_SINGLE
            )

    def test_all_anyonecanpay(self) -> None:
        h = sighash_segwit(
            TX_2IN_2OUT, 0, SCRIPT, self.VALUE, SIGHASH_ALL_ANYONECANPAY
        )
        assert len(h) == 32
...
class TestSerializer:
    ...
    def test_legacy_sighash_all(self) -> None:
        raw = serialize_legacy_tx_for_sighash(
            TX_2IN_2OUT, 0, SCRIPT, SIGHASH_ALL
        )
        assert isinstance(raw, bytes)

    def test_legacy_sighash_none(self) -> None:
        raw = serialize_legacy_tx_for_sighash(
            TX_2IN_2OUT, 0, SCRIPT, SIGHASH_NONE
        )
        assert isinstance(raw, bytes)

    def test_legacy_sighash_single_valid(self) -> None:
        raw = serialize_legacy_tx_for_sighash(
            TX_2IN_2OUT, 0, SCRIPT, SIGHASH_SINGLE
        )
        assert isinstance(raw, bytes)
    ...
    def test_legacy_sighash_single_out_of_range(self) -> None:
        with pytest.raises(ValueError, match="out of bounds"):
            serialize_legacy_tx_for_sighash(
                TX_2IN_2OUT, 5, SCRIPT, SIGHASH_SINGLE
            )
